   ...
   public class Program
    {
        ...
        {
            InstructorDataMapper intstructor_mapper = new InstructorDataMapper();
            var instructors = intstructor_mapper.Select();

            var output = new StringBuilder();
            output.AppendLine("ID\tFirst Name\tLast Name\tRank");
            output.AppendLine("--\t----------\t---------\t-----------");

            //loop through instructor list and display values
            foreach (var instructor in instructors)
            {
                output.Append(instructor.ID.ToString());
                output.Append("\t");
                output.Append(instructor.FirstName.ToString().PadRight(10));
                output.Append("\t");
                output.Append(instructor.LastName.ToString().PadRight(10));
                output.Append("\t"); 
                output.Append(instructor.Rank.ToString().PadRight(10));
                output.AppendLine();
            }
        ...
        private static void ShowStudents()
        {
           ...
           var students = student_mapper.Select();
            ...
            //loop through student list and display values
